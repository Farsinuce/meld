<#
.SYNOPSIS
    Put Meld on the desktop and in the Start menu.

.DESCRIPTION
    Meld ships as a folder you extract anywhere - there is no installer, so nothing has
...
.PARAMETER Startup
    Also start Meld when you log in. It opens straight to the tray, no window.
...
$Exe = Join-Path $Root 'Meld.exe'
$Arguments = ''
$IconPath = Join-Path $Root 'assets\icons\meld.ico'

if (-not (Test-Path $Exe)) {
    $PyW = Join-Path $Root '.venv\Scripts\pythonw.exe'
    $Script = Join-Path $Root 'meld_app.py'
    if ((Test-Path $PyW) -and (Test-Path $Script)) {
        ...
        throw "Could not find Meld.exe or a source checkout under $Root"
    }
}
...
foreach ($target in $Targets) {
    ...
    $lnk.Description = 'Meld - real-world maps into Minecraft worlds'

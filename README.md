...
**Meld 1.9.0 "Unbound"** — a standalone desktop app. Download, run, generate.
...
- **A bake that knows its cost.** Before generating, Meld prices the memory, disk and time a
  region needs against your actual machine — measured numbers, not guesses — and refuses, with
  the fix named, when something won't fit.
...
- [CHANGELOG.md](CHANGELOG.md) — full history; [`docs/`](docs/) — per-release deep dives
